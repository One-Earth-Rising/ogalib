...
#define OGALIB_SCE_THREAD_COUNT 6
#else
#define OGALIB_SCE_THREAD_COUNT 1
#endif
...
static int GetPS5ThreadPriority(f32 priority);
...
ThreadMutex::ThreadMutex(const char* name, ThreadMutexType type):
native(NULL) {
  this->name = name ? name : "";
  ...
  if(native) {
    ...
    default:
      ogalibAssert(false, "Unsupported thread mutex type: %d", type);
      break;
    }
...
void Thread::SetPriority(f32 priority) {
  if(native) {
    ThreadPS5* nativePS5 = static_cast<ThreadPS5*>(native);
...
long long Thread::GetCurrentThreadId() {
...
void ThreadCondition::FinalizeThread(Thread*& thread) {
  if(!thread)
    return;
...
void ThreadCondition::FinalizeThread(Thread*& thread, bool& wait) {
  if(!thread)
    return;
...
int GetPS5ThreadPriority(f32 priority) {
  if(priority == -1.0f)
    return SCE_KERNEL_PRIO_FIFO_DEFAULT;

  f32 value = GetLerp(SCE_KERNEL_PRIO_FIFO_LOWEST, SCE_KERNEL_PRIO_FIFO_HIGHEST, priority);
  return roundf(value);
}
